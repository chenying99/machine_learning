...
if p not in sys.path:
    sys.path.append(p)
import jieba
from constant import PROJECT_DIRECTORY, sogou_classification_data_label_dict, news_sohusite_url_to_label_dict
...
def prepare_sogou_classification_data():
    """
    sogou_classification_data预处理
    """
    ...
    for label in label_dir_list:
        ...
        label_name = sogou_classification_data_label_dict.get(label)
...
sogou_classification_stopwords_set = None


def get_sogou_classification_stopwords_set():
    """
    从文件加载搜狗分类停用词表
    """
    global sogou_classification_stopwords_set
    if not sogou_classification_stopwords_set:
        sogou_classification_stopwords_set = set()
        with open(os.path.join(PROJECT_DIRECTORY, 'data/sogou_classification_stopwords.txt'), 'r') as reader:
            for each_line in reader.readlines():
                word = ensure_unicode(each_line.replace('\n', '').strip().lower())
                sogou_classification_stopwords_set.add(word)
    return sogou_classification_stopwords_set


def segment_sogou_classification_data():
    """
    sogou_classification_data分词
    """
    stopwords_set = get_sogou_classification_stopwords_set()
    read_dir_path = os.path.join(PROJECT_DIRECTORY, "data/sogou_classification_data")
    write_dir_path = os.path.join(PROJECT_DIRECTORY, "data/sogou_classification_segment_data")
...
if __name__ == "__main__":
    ...
    # prepare_sogou_classification_data()
